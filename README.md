...
package implements support for mapping a pair of simultaneously
pressed keys to a command and for mapping the same key being pressed
twice in quick succession to a command. Such bindings are called "key
chords".
...
To move to non visible text in the current buffer, I use the
built-in `isearch-forward` and `isearch-backward` commands.

To operate *on lines* (including moving the cursor around), I use the
...
I use color to know when I've an active `hydra`.
When `handy-line` is *active* the cursor has the color `#f92672`.
When `handy-sexp` is *active* the cursor has the color `#fa87ce`.
...
[tonyaldon/emacs.d/packages/handy.el](https://github.com/tonyaldon/emacs.d/blob/master/packages/handy.el).

You can also find those `hydra` definitions in:

[tonyaldon/emacs.d/init.el](https://github.com/tonyaldon/emacs.d/blob/master/init.el).
